...
extern std::map<uint32_t, uint32_t> data_mo;
enum
{
    TK_NOTYPE = 256, // space
    TK_EQ,           // 等于运算符 "=="
...
uint32_t expr(char *e, bool *success)
{
    ...
    for (int i = 0; i < nr_token; i++)
    {
        ...
        {
            if (i == 0 || tokens[i - 1].type == TK_LPAREN || tokens[i - 1].type == TK_PLUS || tokens[i - 1].type == TK_MINUS || tokens[i - 1].type == TK_MUL || tokens[i - 1].type == TK_DIV || tokens[i - 1].type == TK_EQ || tokens[i - 1].type == TK_NEQ || tokens[i - 1].type == TK_AND)
            {
    ...
    *success = true;
    ...
    else if (p == q)
    {
        ...
        else if (tokens[p].type == TK_REG)
        {
            ...
            {
                return npc_wire.read();
            }else{
                int a;
                sscanf(tokens[p].str, "%d", &a);
                return top->rootp->ysyx_23060286_top__DOT__GRP__DOT__rf[a];
            }
        }
    }
...
int find(int p, int q)
{
    ...
    for (i = p; i <= q; i++)
    {
        ...
        else if (depth == 0 && tokens[i].type != TK_NUM)
        { // 只在最外层处理运算符
