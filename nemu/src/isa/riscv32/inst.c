...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("??????? ????? ????? 011 ????? 00100 11", sltiu  , I, R(rd) = src1 < imm ? 1 : 0); //seqz is no have in risc-v 
  INSTPAT("0100000 ????? ????? 101 ????? 00100 11", srai   , I, R(rd) = (sword_t)src1 >> imm); /// maybe error
  INSTPAT("??????? ????? ????? 111 ????? 00100 11", andi   , I, R(rd) = src1 & imm);
  INSTPAT("??????? ????? ????? 100 ????? 00100 11", xori   , I, R(rd) = src1 ^ imm);
  INSTPAT("??????? ????? ????? 110 ????? 00100 11", ori    , I, R(rd) = src1 | imm);
  INSTPAT("0000000 ????? ????? 101 ????? 00100 11", srli   , I, R(rd) = src1 >> (imm&0x0000001f));
  INSTPAT("0000000 ????? ????? 001 ????? 00100 11", slfli  , I, R(rd) = src1 << (imm&0x0000001f));
  ...
  INSTPAT("0000001 ????? ????? 000 ????? 01100 11", mul    , r, R(rd) = (sword_t)src1 * (sword_t)src2);//why
